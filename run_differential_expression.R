...
# Organism for gene annotation and enrichment analysis
# Options: "human"/"Hs"/"hsa" OR "pig"/"Ss"/"ssc"
# Human databases are far better annotated — use "human" unless gene symbols
# are pig-specific. Since this pipeline uses human-standardized gene symbols,
# "human" gives much better Entrez ID coverage and pathway annotations.
...
cat("Enrichment organism:", ENRICHMENT_ORGANISM, "\n\n")
...
contrast_name <- paste0(FACTOR_NAME, "_", make.names(COMPARISON_LEVEL), "_vs_", make.names(BASELINE_LEVEL))
...
cat(paste0("  - PCA_by_", FACTOR_NAME, ".pdf\n"))
cat("  - PCA_by_Dataset.pdf\n")
cat("  - sample_correlation_heatmap.pdf\n")
cat("  - sample_correlation_heatmap.xlsx (correlation matrix values)\n")
cat("  - MAplot.pdf (main)\n")
