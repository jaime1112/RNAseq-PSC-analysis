...
annotate_results <- function(res, gene_symbols, organism = "human") {
  ...
  # Define columns to retrieve (only request columns available in the database)
  desired_columns <- c("GENENAME", "ENTREZID", "ENSEMBL")
  available_columns <- AnnotationDbi::columns(org_db)
  columns <- intersect(desired_columns, available_columns)
  ...
  if ("GENENAME" %in% columns) res@listData$genename <- ann$GENENAME
  if ("ENSEMBL" %in% columns) res@listData$ensembl <- ann$ENSEMBL
  if ("ENTREZID" %in% columns) res@listData$entrezid <- ann$ENTREZID
