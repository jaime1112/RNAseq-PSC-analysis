...
```r
# Get only one group
group_samples <- get_samples_by_criteria(
  metadata = metadata,
  criteria = list(CellType = "GroupA")
)

# Get specific datasets
dataset_samples <- get_samples_by_criteria(
  metadata = metadata,
  criteria = list(Dataset = c("Dataset1", "Dataset2"))
)
```
...
Example workflow of finding WNT genes across 3 comparisons:
