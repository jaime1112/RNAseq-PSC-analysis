...
| **pheatmap** | Heatmap generation | Heatmaps, sample correlation heatmap |
| **RColorBrewer** | Color palettes | Heatmaps, sample correlation heatmap |
...
- **org.Hs.eg.db** - Required (default for annotation and enrichment via `ENRICHMENT_ORGANISM`)
- **org.Ss.eg.db** - Optional, only needed if `ENRICHMENT_ORGANISM` is set to `"pig"`
