...
### Enrichment Organism Selection

By default, enrichment uses human annotation databases (`org.Hs.eg.db`, KEGG code `hsa`) regardless of the species being studied. This is because:
- Gene symbols in the pipeline are standardized to human nomenclature
- Human databases have far richer GO and KEGG annotations than pig
- This yields substantially better pathway coverage (e.g., stem cell pluripotency pathways)

This is controlled by the `ENRICHMENT_ORGANISM` parameter.

### KEGG Pathway Enrichment

Similar hypergeometric test for KEGG pathways:
- Default organism: "hsa" (human) for best annotation coverage
- Alternative: "ssc" (pig) for pig-specific pathway IDs
...
**Method:**
1. Compute Pearson correlation between all sample pairs
2. Cluster samples using Ward's linkage on 1 - correlation distance
3. Annotate with all available experimental metadata columns

**Implementation:**
- Uses `pheatmap` for visualization
- Symmetric matrix with annotations on both axes
- Useful for detecting batch effects, outlier samples, and verifying group structure

**Interpretation:**
- High correlation within groups: consistent biological replicates
- Low correlation between groups: strong differential expression
- Unexpected low within-group correlation: potential outlier or batch effect
...
3. **Sample-to-Sample Correlation Heatmap**
   ...
   - Annotated by all available metadata columns
   - Identify mismatched or outlier samples
