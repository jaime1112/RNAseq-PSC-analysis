...
# Organism for gene annotation and enrichment
# Use "human" for best coverage — gene symbols are human-standardized
...
Generates PCA plots (by CellType and Dataset) and a sample correlation
heatmap 5. Extracts and annotates results with gene symbols and Entrez
IDs 6. Creates MA plots (overall and pathway-specific) 7. (Optional)
Performs GO/KEGG enrichment analysis 8. (Optional) Filters
pathway-specific genes (e.g., WNT pathway)
...
matrix - `PCA_by_[FACTOR_NAME].pdf` - PCA plot colored by the analysis
factor - `PCA_by_Dataset.pdf` - PCA plot colored by dataset -
`sample_correlation_heatmap.pdf` - Sample-to-sample Pearson correlation
heatmap with clustering, annotated by all metadata columns -
`sample_correlation_heatmap.xlsx` - Correlation matrix values exported
to Excel - `MAplot.pdf` - Main
...
# Examples:
# list(CellType = "GroupA")  # Only one group
# list(Dataset = c("Dataset1", "Dataset2"))  # Specific datasets
```
...
Dataset1,data/Dataset1/counts.txt,
Dataset2,data/Dataset2/counts.txt,ENSG[0-9]+
```
...
Sample1     GroupA      Dataset1   1
Sample2     GroupA      Dataset1   2
Sample3     GroupB      Dataset2   1
```
...
**sample_correlation_heatmap.pdf** - Pearson correlation heatmap of all
samples, clustered by Ward's method, annotated by all metadata columns
(ordered by number of unique values)

**sample_correlation_heatmap.xlsx** - Correlation matrix values exported
to Excel for downstream use
...
**Error: Factor levels not set correctly / model not full rank** -
Ensure BASELINE_LEVEL and COMPARISON_LEVEL match values in the column
specified by FACTOR_NAME - Ensure FACTOR_NAME matches the column used
in DESIGN_FORMULA (e.g., if DESIGN_FORMULA is `~Condition`, then
FACTOR_NAME must be `"Condition"`) - Values are case-sensitive
...
1.  **Always check PCA plots and the sample correlation heatmap first** -
    Ensure samples cluster as expected and correlations are consistent
    within groups
